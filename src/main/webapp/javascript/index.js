$(document)
		.ready(
				function() {
					...
					var source = null; // used to open Server Sent Event
										// connection
					...
					// this function makes the server apply changes about the
					// player's preferences
					// about the map and about the game configuration.
					function applyChanges() {
						var isMapDefault = $(
								'input[name=mapConfigRadio]:checked',
								'#mapForm').val() == "default";
						var objectiveType = $(
								'input[name=modalityConfigRadio]:checked',
								'#modalityForm').val();
						var jsonRequestPayload = "{'objective': '"
								+ objectiveType + "','difficulty': '";
						if (isMapDefault) {
							jsonRequestPayload += $("#difficultySelect").val()
									+ "'}";
						} else {
							jsonRequestPayload += "custom', "
									+ $("#jsonTextArea").val() + ", ";
							jsonRequestPayload += "'mapSVG': '"
									+ $("#svgTextArea").val() + "'}";
						}
					...
					// this function is called whenever the player joins the
					// match
					function joinGame() {
					...
					// this function is called whenever the player exits from
					// the match
					function exitGame() {
						$.get("./match/exit");
						if (source != null)
					...
					// function used to communicate important messages to the
					// player
					...
					// this function checks whether there are more than two
					// players or not
					function areThereTwoPlayers(playersArray) {
						return playersArray.length >= 2;
					}

					// this function refreshes the table with the most recent
					// changes
					function refreshPlayersTable(playersArray) {
						$("tr").remove();
						$("#playersTable")
								.append(
										"<tr><th>Name</th><th>Color</th><th>Ready</th></tr>");
						for (var i = 0; i < playersArray.length; ++i) {
							$("#playersTable").append(
									"<tr><td>" + playersArray[i].nickname
											+ "</td>" + "<td>"
											+ playersArray[i].color + "</td>"
											+ "<td>" + playersArray[i].ready
											+ "</td></tr>");
						}
					}
				});